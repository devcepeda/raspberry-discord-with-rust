use super::shared::{
    ...
    ensure_video_under_limit, ffmpeg_available, ffprobe_available, purge_previous_temp_dirs,
    run_command_capture, select_final_media_file, send_file, simplify_yt_dlp_error,
    yt_dlp_available,
};
...
pub(super) async fn download_video_and_send(ctx: &Context, msg: &Message, url: &str) {
    ...
    let output = match primary {
        ...
        Ok(out) => {
            ...
            if details.to_ascii_lowercase().contains("conversion failed") {
                let _ = msg
                    .channel_id
                    .say(
                        &ctx.http,
...
async fn run_video_download(
    ...
    if force_mp4_postprocess {
        args.push("--merge-output-format".to_string());
        args.push("mp4".to_string());
    }

    args.push("-f".to_string());
    args.push("b/bv*+ba".to_string());
